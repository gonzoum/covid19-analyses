...
This chapter has details on where the data came from and how to garther these data.  If you are not interested in these details, it is fine to skip this chapter.
...
I prefer not to have data files be in the git repository as a matter of habit (e.g., data may contain private information yet the repository may be public).
...
Now I have two datacov files, World and US, and I can use each as need in the subsequent code.  The main thing left to "fix" is that the new datacov.US is broken down by county and my code is currently written for state.  Later, I'll sum over county to get state counts, but will keep the county level information for possible use in later examples using more granular county-level information.
