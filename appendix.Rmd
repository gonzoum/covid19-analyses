...
## Once Installed Run these Commands (both PC and Mac)
...
There are more systematic approaches to creating self-contained environments with the packae [renv](https://rstudio.github.io/renv/articles/renv.html) or solutions outside R such as [docker containers](https://www.docker.com/?utm_source=google&utm_medium=cpc&utm_campaign=dockerhomepage&utm_content=namer&utm_term=dockerhomepage&utm_budget=growth&gclid=CjwKCAjw7LX0BRBiEiwA__gNwx4_6mBagzF3TIvDIHw-BJixzj6KQAHgr5conMNwazJc-JI_uNeLqBoC-ZEQAvD_BwE).

# Appendix C: Saving Workspace

Save workspace 
so data and objects can be retrieved at later time. This is helpful for debugging. Normally not something I do but given data are changing so often might as well keep a snapshot of the data according to each release. I may decide to stop saving the data.  File name will change when I change the version number on git (e.g., v0.9). This code assumes you have a folder called data-archive to place these saved workspaces.
...
# Appendix D: The bookdown package

This entire html site was built using the bookdown R package. It has advanced features for building reports around analysis as you can see. The [bookdown manual](https://bookdown.org/yihui/bookdown/) was written with the bookdown package.  I uploaded the entire file structure to my git repository so you can build this bookdown website yourself.  To do production builds I click on Build/Build All, which starts up a new R session compiles all the R code and converts to both html and pdf version.  

While working, writing code and debugging I build the code with the R command bookdown::serve_book(in_session=T), which runs all the Rmd files locally in my session so I have all the R objects available in my R session, and can display the new version on a browser every time I save the Rmd file I'm working on.  Sometimest to debug I may have to run R chunks manually to get the data set to the point at which the error occurs. To help quickly running Rmd files earlier in the sequence to get me to the place where the error occurs, I can run the command xaringan::inf_mr() for each Rmd file (make the Rmd file active in the RStudio editor before running inf_mr()) and be careful with multiple serve sessions.
