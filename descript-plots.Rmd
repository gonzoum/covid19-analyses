...
Here is the daily incidence rate using the same US data.   The curve does not appear to be flattening yet.
...
allstates.temp[,5:(ncol(temp)+4)] <- temp

temp <- apply(allstates.temp[,-c(1:4)],2,sum,na.rm=T)
allstates.temp[1,5:(length(temp)+4)] <- temp
allstates.temp <- allstates.temp[1,]
temp.long <- gather(allstates.temp, day, count, `3/10/20`:ncol(allstates.temp))
