...
# something wrong with day 56 and 57 so need to double check this; omit for now
covid.tracking.new <- subset(covid.tracking.new, day.numeric != 56 & day.numeric != 57)
out <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=46, b0=-2.8,b1=.1), verbose=T, groups=~state, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=500) )
```
