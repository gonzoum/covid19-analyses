...
out.nooutlier <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=subset(covid.tracking.new, deathIncrease<800),start=c(a=40, b0=-1.8,b1=.03), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001, maxIter=1000) )
...
covid.tracking.new.nooutlier$predicted.nooutlier <- temp.predict
...
starts <- getInitial(death~ SSlogis(day.numeric, Asym=1, xmid=1, scal=1), data=covid.tracking.new)
tempdata <- covid.tracking.new[,c("day.numeric","death","state","deathIncrease")]
out.logistic.death <- nlme(death ~ SSlogis(day.numeric, asym,xmid,scal), fixed = asym+xmid+scal~1,
            random =  asym+xmid+scal~1, data=na.omit(tempdata),start=c( asym=starts[1], xmid=starts[2],scal=starts[3]), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.mas=1000) )
```
