...
out <- nlme(deathIncrease.pc ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=10, b0=1.75,b1=-.01), verbose=T, groups=~state, control=nlmeControl(msMaxIter=400,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=400, tolerance=.5, msTol=.01) )
...
out.first42days <- nlme(deathIncrease.pc ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b0+b1~1, data=subset(covid.tracking.new, day.numeric<43),start=c(a=12, b0=-2.7,b1=.072), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, niterEM=1000, msMaxEval=1000, tolerance=1, msTol=.1) )



```

Play using Newton's cooling equation; dropped one random effect due to convergence issues (come back to this later)
...
out.43days <- nlme(deathIncrease.pc ~ richfu.heat(day.numeric,b0,b1,b2), fixed = b0+b1+b2~1,
            random =  b0+b1~1, data=subset(covid.tracking.new, day.numeric>=34),start=c(b0=2.5,b1=4,b2=-.055), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001, maxIter=1000, tolerance=.01, msTol=.001) )
...
out.logistic.death <- nlme(death.pc ~ SSlogis(day.numeric, asym,xmid,scal), fixed = asym+xmid+scal~1,
            random =  xmid+scal~1, data=na.omit(tempdata),start=c( asym=starts[1], xmid=starts[2],scal=starts[3]), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=1, maxIter=1000, eval.mas=1000, tolerance=1) )
```
