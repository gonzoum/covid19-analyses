...
out <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=47.2, b0=-2.3,b1=.02), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=1000) )
```
...
out.nls <- nls(deathIncrease ~ richfu.sir3(day.numeric, a, b01,b11,b02,b12), data=na.omit(subset(covid.tracking.new[,c("state","deathIncrease","day.numeric")], state=="NY")), start=list(a=720, b01=-2.29,b11=.06, b02=3, b12=.06))

out.nls <- nls(deathIncrease ~ richfu.sir3(day.numeric, a, b01,b11,b01,b11), data=na.omit(subset(covid.tracking.new[,c("state","deathIncrease","day.numeric")], state=="NY")), start=list(a=800, b01=-2,b11=.08))

 with(temp, plot(day.numeric, richfu.sir3(day.numeric,1200, -2, .08, -2.75,.09)))
 with(temp, points(day.numeric, deathIncrease,col="red"))

out <- nlme(deathIncrease ~ richfu.sir3(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=43, b0=-2.,b1=.04), verbose=T, groups=~state, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=500) )
