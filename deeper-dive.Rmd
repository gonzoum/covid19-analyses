...
out <- nlme(deathIncrease.pc ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b0+b1~1, data=covid.tracking.new,start=c(a=8, b01=.9,b1=-.01), verbose=T, groups=~state, control=nlmeControl(msMaxIter=400,opt="nlminb",pnlsTol=.1,msVerbose=T,minScale=.001, maxIter=400, tolerance=.01, msTol=.01) )
#check public for recent run 7, 1.6, -.02, I think I had 7, 1.8 -.03
...
out.43days <- nlme(deathIncrease.pc ~ richfu.heat(day.numeric,b0=0,b1,b2), fixed = b1+b2~1,
            random =  b1+b2~1, data=subset(covid.tracking.new, day.numeric>=34),start=c(b1=10.7,b2=-.029), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.1,msVerbose=T,minScale=.1, maxIter=1000, tolerance=1, msTol=.1) )
