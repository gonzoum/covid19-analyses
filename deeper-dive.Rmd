...
# focus just on counties with cases > 400 as of the compilation of the report
...
subset(georgia.df.max, day > as.Date("2020/04/1") & maxcount>=400) %>% 
  mutate(label = if_else(day.numeric == max(day.numeric), as.character(Admin2), NA_character_)) %>%
ggplot(aes(day,count, group=Admin2, color=Admin2)) + geom_line() + ggtitle("Georgia counties with greater than 400 cases")  + geom_vline(xintercept=as.Date("2020/04/24"), color="red") + ylim(range.ylim) + theme_bw() +
...
subset(alabama.df.max, day > as.Date("2020/04/1") & maxcount>=400) %>% 
  mutate(label = if_else(day.numeric == max(day.numeric), as.character(Admin2), NA_character_)) %>%
ggplot(aes(day,count, group=Admin2, color=Admin2)) + geom_line() + ggtitle("Alabama counties with greater than 400 cases")  + geom_vline(xintercept=as.Date("2020/04/24"), color="red") + ylim(range.ylim) + theme_bw() +
   geom_label_repel(aes(label= label), nudge_x=2, na.rm=T,segment.color = 'grey90', label.size=.01, size=2.5, show.legend=F) + scale_x_date(limits=c(as.Date("2020/04/1"), as.Date(max.day+5))) +
    theme(legend.position="none", plot.margin = margin(0.1, 1, 0.1, 0.1, "cm"))
```
...
Let's perform a direct fit to the daily death count, since death is a more appropriate outcome for the SIR model.  For this section I'll use the covid.tracking data (see Chapter \@ref(readintro)). We haven't worked much with that data yet so I'll also need to do some data cleaning and formatting to be consistent with the other analyses conducted in this book.
...
```{r results="hide"}
#out <- nls(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), data=covid.tracking.new,start=c(a=155, b0=-1.9,b1=.001), algorithm="port")
out <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=40, b0=-1.8,b1=.03), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001, maxIter=1000) )

#refit omiting the outlier
out.nooutlier <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=subset(covid.tracking.new, deathIncrease<800),start=c(a=40, b0=-2,b1=.01), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001, maxIter=1000) )
temp.data <- covid.tracking.new
dropped.data <- which(covid.tracking.new$deathIncrease>=800)

out.first42days <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=subset(covid.tracking.new, day.numeric<43),start=c(a=115, b0=2.1,b1=-.01), verbose=T, groups=~state, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001, maxIter=1000) )
...
ggplot( subset(covid.tracking.new, state=="NY"), aes(x=day.numeric,y=deathIncrease)) + 
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
  geom_line(aes(day.numeric, predicted.nooutlier), color="blue") +
...
```{r  }
p1 <- ggplot( subset(covid.tracking.new, state=="NJ"), aes(x=day.numeric,y=deathIncrease)) +
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
    labs(y="Daily incidence ",
...
p2 <- ggplot( subset(covid.tracking.new, state=="MI"), aes(x=day.numeric,y=deathIncrease)) +  
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
    labs(y="Daily incidence ",
...
```{r }
p1 <- ggplot( subset(covid.tracking.new, state=="NY"), aes(x=day.numeric,y=deathIncrease)) + 
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
    labs(y="Daily incidence ",
                                   title="Daily Covid-19 Deaths in New York")  +
    theme(legend.position = "none",
          strip.text.y = element_text(size=11))  + ylim(0,1000)

p2 <- ggplot( subset(covid.tracking.new, state=="NJ"), aes(x=day.numeric,y=deathIncrease)) +  
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
    labs(y="Daily incidence ",
                                   title="Daily Covid-19 Deaths in New Jersey")  +
...
p3 <- ggplot( subset(covid.tracking.new, state=="MI"), aes(x=day.numeric,y=deathIncrease)) + 
  #  scale_x_date(date_breaks="1 week", date_labels = "%d %b") +
  geom_line(aes(day.numeric, predicted), color="red") + geom_point() +
    labs(y="Daily incidence ",
                                   title="Daily Covid-19 Deaths in Michigan")  +
...
out.nls <- nls(deathIncrease ~ richfu.sir3(day.numeric, a, b01,b11,b02,b12), data=na.omit(subset(covid.tracking.new[,c("state","deathIncrease","day.numeric")], state=="NY")), start=list(a=47, b01=-2.1,b11=.01, b02=-2.1, b12=.01))

out.nls <- nls(deathIncrease ~ richfu.sir(day.numeric, a, b01,b11), data=na.omit(subset(covid.tracking.new[,c("state","deathIncrease","day.numeric")], state=="NY")), start=list(a=800, b01=-2.1,b11=.01))
 with(temp, plot(day.numeric, richfu.sir(day.numeric,698, -2.14, .06)))

out <- nlme(deathIncrease ~ richfu.sir2(day.numeric, a, b0,b1,b2), fixed = a+b0+b1+b2~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=46.5, b0=-2.1,b1=.05, b2=500), verbose=T, groups=~state, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=500) )


 with(temp, plot(day.numeric, richfu.sir3(day.numeric,.391, -1.9, .008, -1.3, .2)))
 with(temp, points(day.numeric, deathIncrease,col="red"))

out <- nlme(deathIncrease ~ richfu.sir3(day.numeric, a, b0,b1,b0,b12), fixed = a+b0+b01+b1+b12~1,
            random =  a~1, data=covid.tracking.new,start=c(a=.4, b0=-1.9,b01=-1.3, b1=.008,b12=.2), verbose=T, groups=~state, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=500) )
