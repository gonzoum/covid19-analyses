...
day.death.totals.US <- covid.tracking.new %>% group_by(date) %>% summarise(daytotal=sum(deathIncrease,na.rm=T))
...
day.hospitalizedCurrently <- covid.tracking.new %>% group_by(date) %>% summarise(daytotal=sum(hospitalizedCurrently,na.rm=T))

day.inIcuCurrently <- covid.tracking.new %>% group_by(date) %>% summarise(daytotal=sum(inIcuCurrently,na.rm=T))

day.onVentilatorCurrently <- covid.tracking.new %>% group_by(date) %>% summarise(daytotal=sum(onVentilatorCurrently,na.rm=T))
