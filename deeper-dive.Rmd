...
ggplot(subset(georgia.df.max, day > as.Date("2020/04/1") & maxcount>=250), aes(day,count, group=Admin2, color=Admin2)) + geom_line() + ggtitle("Georgia counties with greater than 250 cases")  + geom_vline(xintercept=as.Date("2020/04/24"), color="red") + ylim(range.ylim)
...
ggplot(subset(alabama.df.max, day > as.Date("2020/04/1") & maxcount>=250), aes(day,count, group=Admin2, color=Admin2)) + geom_line() + ggtitle("Alabama counties with greater than 250 cases")  + geom_vline(xintercept=as.Date("2020/04/24"), color="red") + ylim(range.ylim)
...
$$
where sech is the hyperbolic secant, its argument is a linear transformation of time t with a slope and intercept denoted by $\kappa$s, and parameter $a$ is a multiplier.  These parameters are each functions of the parameters of the SIR model (the $\beta_1$ and $\beta_2$ from Chapter \@ref(process)) as well as the starting count at time 0. The hyperbolic secant can be reexpresssed in terms of exponentials:  $\mbox{sech} (x) = \frac{2}{\exp^{x} + \exp^{-x}}$ Let's perform a direct fit to the daily death count, since death is a more appriate outcome for the SIR model.  For this section I'll use the covid.tracking data (see Chapter \@ref(readintro)). We haven't worked much with that data yet so I'll also need to do some data cleaning and formating to be consistent with the other analyses conducted in this book.
...
#something is wrong with data on 4/29 as this won't converge;drop 4/29 
#save into temp and put temp in nlme call as a temporary fix
temp <- subset(covid.tracking.new, day.numeric!=51)
covid.tracking.new <- temp
out <- nlme(deathIncrease ~ richfu.sir(day.numeric, a, b0,b1), fixed = a+b0+b1~1,
            random =  a+b1~1, data=covid.tracking.new,start=c(a=45, b0=-2.8,b1=.07), verbose=T, groups=~state, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=500) )
```
...
covid.tracking.new$predicted <- predict(out)
...
But a more interesting result emerges by examining this approximate functional form.  We can integrate this approximation over t to see what form is given for the cumulative death count. in this context, integration is analogous to summing the daily counts to create a cumulative count but we do this on the function rather than the data in order to learn the form of the function on the cumulative count. 
...
k0 <- unlist(coef.ny[2])
k1 <- unlist(coef.ny[3])
t <- rev(covid.tracking.new$day.numeric[covid.tracking.new$state=="NY"])
pred <- a*(exp(2*k1*t + 2*k0) - 1)/(k1*(exp(2*k1*t+2*k0)+1))
#set integration constant so incidence at time 1 is 0
pred <- pred+(-1*min(pred))+min(covid.tracking.new$death[covid.tracking.new$state=="NY"],na.rm=T)
...
#add predicted values from the logistic regression
