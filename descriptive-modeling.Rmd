...
out <- nlme(count ~ richfu.logistic(day.numeric, l,y,b1), fixed = l+y+b1~1,
            random =  l+y~1, data=na.omit(allstates.long.new),start=c( l=25000,y=160,b1=-.1), verbose=T, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.max=1000) )
```
...
out <- nlme(count ~ richfu.logistic(day.numeric, l,y,b1), fixed = l+y+b1~1,
            random =  l+y~1, data=na.omit(allstates.long.new),start=c( l=25000,y=160,b1=-.1), verbose=T, correlation=corARMA(form=~1,p=1,q=1), control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.max=1000) )
```
...
out <- nlme(count ~ richfu.logistic(day.numeric, l,y+b2*povertyperc,b1), fixed = l+y+b1 + b2~1,
            random =  l+y~1, data=na.omit(allstates.long.new),start=c( l=17500,y=200,b1=-.15, b2=-4.5), verbose=T,  control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.02,msVerbose=T,minScale=.02, maxIter=1000, eval.max=1000) )
```
...
Pending:  examine the nonlinear mixed model with a Poisson distribution; code needs to be checked;
...
of the forecast package and the excellent outline book by [Hyndman and Athanasopoulos](https://otexts.com/fpp2/). I'll forecast 10 days out from the current date using New York data and compute an error band, show how to check residuals with the  checkresiduals() command, and how to incorporate use hierarchical time series to model state trajectories within the US.  
...
I will illustrate the use of Bayesian nonlinear regression modeling to estimate prediction uncertainty. The Bayesian approach also allows for more general models than the standard regression approaches described earlier in this chapter, these include expanding the assumptions limitations of the classic approach.  For example, within a Bayesian approach one can include predictors of the variance terms, which is not easy to do in the classic setting. This could be useful, for example, if a predictor influences not only the value of a random effect parameter but also its variance. Or, we may be able to do a better job of developing models that are more appropriate for the count data we are using throughout these notes.  We know that a normal distribution can provide a reasonable approximation to count data distributions but there may be cases, such as the present nonlinear random effects case, where that approximation breaks down and we would be better off modeling the count distribution directly.  Another example of something that is difficult to do in the classical framework is dealing with distribution of random effects that do not follow a normal distribution. Earlier we saw that there may be two clusters of states and that the joint distribution of random effect parameters appears to be skewed.  This makes suspect inferences based on random effects that assume a joint normal distribution. These nonstandard cases can be handled relatively easy in the Bayesian framework.

I co-wrote an [introductory chapter](http://www-personal.umich.edu/~gonzo/papers/feinberg-gonzalez-2012-bayesian.pdf) on Bayesian statistics with [Fred Feinberg](https://michiganross.umich.edu/faculty-research/faculty/fred-feinberg) that highlights some of the advantages and disadvantages of the Bayesian framework.  For a complete book on Bayesian statistics I very much like [Gelman et al. Bayesian Data Analysis](https://www.amazon.com/Bayesian-Analysis-Chapman-Statistical-Science-ebook/dp/B00I60M6H6/ref=sr_1_2?dchild=1&keywords=Gelman+Bayesian+statistics&qid=1587941657&sr=8-2).  Other textbooks include [Kruschke's Doing Bayesian Data Analysis](https://www.amazon.com/Doing-Bayesian-Data-Analysis-Tutorial-ebook/dp/B00PYZ2VR6/ref=sr_1_6?dchild=1&keywords=Krusce+bayesian+statistics&qid=1587941724&sr=8-6).

My single Bayesian example here will take a problem that was extremely difficult in what we did earlier but is relatively easy to do in the Bayesian context:  put intervals (or, as a Bayesian would say, credible intervals because the interpretation is slightly different than a standard confidence interval) around predictions form a nonlinear random effects regression model.  This is an important detail that the classical approach has a difficult time addressing, as I mentioned in earlier subsections of this chapter when I wasn't able to establish the uncertainty around the predictions.  While uncertainty intervals around the parameter estimates are possible in the classical approach, computing error bars around the prediction interval in a random effects nonlinear model is not straightforward. 
...
fit_loss <- brm(
  ...
  prior = c(
    prior(normal(15000, 40000), nlpar = "asymp"),
    prior(normal(40, 40), nlpar = "liftoff"),
    prior(normal(.2, .5), nlpar = "b1", lb=0)
    ), iter=2000,
...
```{r eval=F, echo=F}
# need to fix, interals are too narrow
...
#ggplot(temp, aes(day.numeric, Estimate)) + geom_smooth(aes(ymin=Q2.5, ymax=Q97.5), stat="identity") 
...
ggplot(subset(allstates.long, state.abb %in% c("Michigan"))) +
  aes(x = day.numeric, y = count) +
  geom_point(color = "red") + stat_lineribbon(data=subset(fits, state.abb %in% c("Michigan")), aes(y=.prediction), .width = c(.95,.80, .50), alpha=1/4, color = brewer.pal(5, "Blues")[[5]]) +
...
ggplot(subset(allstates.long, state.abb %in% c("New York","Michigan"))) +
  aes(x = day.numeric, y = count) +
  geom_line(
    aes(group = interaction(state.abb, .draw), y = .value),
    data = subset(fits, state.abb %in% c("New York","Michigan")), alpha = .3) +
  geom_point(color = "red") +
  facet_wrap("state.abb")
...
I adapted some of the code from this [webpage](https://towardsdatascience.com/state-space-model-and-kalman-filter-for-time-series-prediction-basic-structural-dynamic-linear-2421d7b49fa6). R example pending.
