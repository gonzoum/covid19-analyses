...
pred <- data.frame(nydf, predictInterval(out.lmer, nydf, level=.95))
pred
...
pred <- data.frame(midf, predictInterval(out.lmer, midf))
pred
...
out <- nlme(percent ~ richfu(day.numeric, l=0, rate,y,b1=1), fixed = rate+y~1,
            random =  rate+y ~1, data=na.omit(allstates.long.new),start=c( rate=1.2,y=.001), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.0001,msVerbose=T,minScale=.0001) )
```
...
#XXX 4/13/20 data error intervals causes error (need to fix); no variance on the rate parameter
...
```{r messages=F, warnings=F, results="hide"}
...
#confidence intervals of the parameters
#had issues with random effect intervals on 4/6/20 so for now set CI for only fixed
intervals(out.logistic, which="fixed")
...
```{r messages=F, warning=F, eval=F, echo=F}
...
```{r messages=F, warning=F, results="hide", echo=T}
out <- nlme(count ~ richfu.logistic(day.numeric, l,y,b1), fixed = l+y+b1~1,
...
```{r cache=cache, warning=F, message=F}
#set.seed(03302020)
