...
Now that we understand the slopes and intercepts, let's make some predictions.  I'll go 5 days out.  The first plot will be on the log scale.
...
You can think of this model as being similar to a balance in a savings account that builds with compound interest. At each time step, a percentage is added to the current principle thus increasing the principle. At the subsequent time step, the principle is now higher so the interest that day will also be higher.  In the case of the covid-19 counts, each day the total grows by some percentage, like the principle in the savings account.
...
An interesting use of machine learning methods (specifically computer vision) is the development of a physical distancing index by a group at UM.  Physical movement of humans and social distancing is detected in major cities. This [website](https://pdi.voxel51.com/) has some live cameras and charts to illustrate their index.
