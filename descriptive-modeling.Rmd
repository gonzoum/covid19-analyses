...
```{exercise}
Use the anova command to compare the fits of the regressions with equal and unequal state error variances. THis is is an increment in fit test comparing the reduced model (e.g., one residual variance for all states) to a full model (e.g., each state has its own residual variance). The command would be anova(reduced, full) where you replace reduced and full with the name of the object you saved the nlme (e.g., reduced = out.logistic in the example below).
```

```{exercise}
Re-run the logistic growth nonlinear mixed model using the percentage of the state population that tests positive. Compare these results to the logistic growth models computed on the count data with equal residual variance across states. How do you address the different scales of the dependent variable?
```
...
of the forecast package and the excellent outline book by [Hyndman and Athanasopoulos](https://otexts.com/fpp2/). I'll forecast 10 days out from the current date using New York data and compute an error band, show how to check reisudals with the  checkresiduals() command, and how to incorporate use hierarchical time series to model state trajectories within the US.  
...
I will return to state-space models in Chapter \@ref(process) when discussiing process models. 
