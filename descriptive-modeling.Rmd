...
#            random = Asym + xmid + scal~ 1, data=na.omit(allstates.long.new),start=c(Asym=starts[1], xmid=starts[2], scal=starts[3]), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.1,msVerbose=T,minScale=.1, maxIter=400, eval.max=400, tolerance=.1))

#drop random effect scal for now 7/6/2020
 out.perc <- nlme(percap10000 ~ SSlogis(day.numeric, Asym, xmid,scal), fixed =  Asym + xmid + scal  ~1,
            random = Asym + scal ~ 1, data=na.omit(allstates.long.new),start=c(Asym=115, xmid=80, scal=21), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=400, eval.max=400, tolerance=.1))
```
