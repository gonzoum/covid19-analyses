...
#            random =  l+b1~1, data=na.omit(allstates.long.new),start=c( l=31700,y=64,b1=-.08), verbose=T, #control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, #eval.max=1000) )
 ...
 out <- nlme(count ~ SSlogis(day.numeric, Asym, xmid,scal), fixed =  Asym + xmid + scal  ~1,
           random =  Asym + scal + xmid~ 1, data=na.omit(allstates.long.new),start=c(Asym=25000, xmid=35, scal=9), verbose=T, control=nlmeControl(msMaxIter=500))
#c(Asym=starts[1], xmid=starts[2],scal=starts[3])
```
...
```{r}
par(pty="s")
plot(allstates$povertyperc, coef(out.logistic)[,2],xlab="State Poverty Percentage", ylab="random effect parameter b1 in logistic growth")
...
out <- nlme(count ~ SSlogis(day.numeric, Asym, xmid+b2*povertyperc, scal), fixed = Asym + xmid + scal+b2~1,
            random =  Asym  + xmid~1, data=na.omit(allstates.long.new),start=c( Asym=28500,xmid=50,scal=9, b2=-.5), verbose=T,  control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.max=1000) )
```
...
Need to edit this section as function changed.
```{r}
...
```{r cache=F, results=ifelse(longform, "markup", "hide")}
#set.seed(03302020)
