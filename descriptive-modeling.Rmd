...
Now lets look plots of residuals to diagnose potential issues.
...
pred <- data.frame(nydf, predictInterval(out.lmer, nydf))
pred
...
```
Note that the linear pattern (line and prediction band) doesn't do a good job of capturing the actual data points. Some points are under predicted in the early days, then overpredicted, then underpredicted again. This would produce a patter where the residuals are negative, then positive, then negative again as we saw in one of the residual plots. 
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
  ...
  annotate(geom="text",x=max.day.numeric-4,y=-5, label="Days with observations", color="red") +
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
    theme(legend.position="none", plot.margin = margin(0.1, 1, 0.1, 0.1, "cm")) + geom_vline(xintercept=max.day.numeric+.5,color="red") +
  annotate(geom="text",x=max.day.numeric-4,y=4, label="Days with observations", color="red") +
  annotate(geom="text",x=max.day.numeric+4,y=4, label="Predictions", color="red") 
```

<!-- Those curves look interesting, but let's pick out specific states and look at some numbers. I'll show the current date, tomorrow and 5 days out for New York state.  -->

```{r eval=F, echo=F}
day.plus.1 <-          predmat[predmat$day.numeric==max.day.numeric+1,]          %>% mutate(prediction = (10^prediction)*population/100)
day.plus.days.ahead <- predmat[predmat$day.numeric==max.day.numeric+days.ahead,] %>% mutate(prediction = (10^prediction)*population/100)
...
```{r loglinearplot, fig.cap="New York counts with Prediction 5 days beyond observed data", echo=F, eval=F}
...
manualcounts <- data.frame(day.numeric= c((max.day.numeric+1):(max.day.numeric+9)), fit = c(20884, 25681,30841,37877,44876, 52410,  59648, 66663,75833 )) 
...
The polynomial idea to approximate functions can be extended in more complex ways. [Here](http://uc-r.github.io/mars) is a tutorial using the MARS package in R.
...
Now let's plot  the fitted values of each state.
...
Here is the predicted curve for New York. We know the residuals suggest the model is 'off' for NY (systematically under and over predicting), but this plots suggests a logistic is a reasonable fit, and most of the part that is off is that it didnt' chapter the liftoff and sharp increase once the curve started to increase.
...
Just to illustrate feature extraction here is the output of the daily count variable for each of the 50 states
