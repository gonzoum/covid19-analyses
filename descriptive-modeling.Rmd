...
if (is_latex_output())
   stargazer(out.logistic, header=F, type="latex",report="vcst",single.row=T,omit.stat="n",  omit.table.layout = "n", digits=2, digits.extra=2)
...
out <- nlme(count ~ SSlogis(day.numeric, Asym, xmid+b2*povertyperc, scal), fixed = Asym + xmid + scal+b2~1,
            random =  Asym  + xmid~1, data=na.omit(allstates.long.new),
            start=c( Asym=start.vals[1],xmid=start.vals[2],scal=start.vals[3], b2=-.5), verbose=T,  
            control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.max=1000) )
```
