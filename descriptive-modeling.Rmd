...
There is evidence of skewness in this variable, which can raises concern about the distributional assumptions. 
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
  ...
  annotate(geom="text",x=max.day.numeric-10,y=-5, label="Days with observations", color="red") +
  annotate(geom="text",x=max.day.numeric+6,y=-5, label="Predictions", color="red") 
```
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
    theme(legend.position="none", plot.margin = margin(0.1, 1, 0.1, 0.1, "cm")) + geom_vline(xintercept=max.day.numeric+.5,color="red") +
  annotate(geom="text",x=max.day.numeric-10,y=16, label="Days with observations", color="red") +
  annotate(geom="text",x=max.day.numeric+6,y=16, label="Predictions", color="red") 
```
...
For example, if you can include interactions between predictors or controls using other predictors. 
...
This type of model where we transform the dependent variable by converting to logs, creates an additive error term in the log scale (i.e., log(counts) = f(days) + $\epsilon$). But maybe that isn't the right error structure. We can study the residuals to see if there are nonlinearities. It maybe  the error is added to the counts rather than the log of the counts (i.e., the data may exhibit a  counts = f(days) + $\epsilon$ structure, which is a different error structure than the one on the log scale).  
...
out <- nlme(percent ~ richfu(day.numeric, l=0, rate,y,b1=1), fixed = rate+y~1,
            random =  rate+y ~1, data=na.omit(allstates.long.new),start=c( rate=1.05,y=.01), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01) )
```
...
The fixed effects parameters refer to state-level parameters. You can think of these as the typical values for the states.  The random effects part refers to the variability across states in those parameters. For example, while the fixed effects rate parameter is estimated at `r round(fixef(out.exponential)[1], 2)`, the state deviations around `r round(fixef(out.exponential)[1], 2)` have a standard deviation of `r round(as.numeric(VarCorr(out.exponential)[1,2]),3)`.
...
We can examine specific states. The best fitting exponential curve for New York with the actual data points is shown in the next figure.
...
Let's fit the logistic function to these data.  Note that I reverted to counts and do not use state population size in this model. 

```{asis echo=longform}
I could incorporate population by including a weight parameter (weights=varIdent(form= ~1|state.abb)), which allows each state to have its own residual variance, or change the dependent variable from raw counts to, say, per capita counts.  I'll leave that as an exercise for the reader to check on how that changes the results. 
```

```{r messages=F, warnings=F, results="hide", echo=longform}
# general form but fix b2 = 0
out <- nlme(count ~ richfu.logistic(day.numeric, l,y,b1,b2=0), fixed = l+y+b1~1,
            random =  l+b1~1, data=na.omit(allstates.long.new),start=c( l=30100,y=68,b1=-.05), verbose=T, control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=1000, eval.max=1000) )
```
...
# out2 <- nlme(count ~ SSlogis(day.numeric, Asym, xmid,scal), fixed =  Asym + xmid + scal  ~1,
...
```{exercise}
Re-run the logistic growth nonlinear mixed model using the percentage of the state population that test positive. Compare these results to the logistic growth models computed on the count data with equal residual variance across states. How do you address the different scales of the dependent variable?
```
...
out <- nlme(count ~ richfu.logistic(day.numeric, l,y,b1,b2=0), fixed = l+y+b1~1,
            random =  l+b1~1, data=na.omit(allstates.long.new),start=c( l=25000,y=160,b1=-.1), verbose=T, correlation=corARMA(form=~1,p=1,q=1), control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=1000, eval.max=1000) )
```
...
For illustration purposes, here is another way of fitting this model using a different package lme4.  While lme4 is in some ways easier to use, it is not as flexible as nlme, which allows, for example, correlated error structure on the residuals and the ability to impose structure on the covariance matrix between random effects.  In this example, I'll also illustrate the use of the self-starting function (available both in nlme and lme4), which can speed up the process of finding adequate starting values as well as make the estimation more efficient.  To mix things up, in this example I'll use the percentage of state population that tests positive and I can quickly get a new set of starting values.


```{r messages=F, warning=F, echo=longform}
# I'll only provide a summary and a residual plot.
# SSlogis uses this functional form for logistic growth
# Asym/(1+exp((xmid-input)/scal))

# one way to get starting values
#getInitial(percent ~ SSlogis(day.numeric, l,y,b1), data=na.omit(allstates.long.new))
...
#temp <- nls(percent ~ SSlogis(day.numeric, l, y,b1), data=na.omit(allstates.long.new))
#coef(temp)

out.logistic.nlmer <- nlmer(percent ~ SSlogis(day.numeric, l, y,b1) ~ l + y | state.abb, data=na.omit(allstates.long.new), start=c( l=.371, y=39, b1=7),    control=nlmerControl(tolPwrss=.01))
...
ggplot(nydf.nlmer, aes(day.numeric,predict,group=state.abb, color=state.abb )) + geom_line() + geom_point(aes(x=day.numeric, y=percent), col="blue") + ggtitle("New York: Actual counts in blue; logistic growth estimated by lme4 in red")
```


### Additional Predictors: Example with State-Level Poverty Rate

Now that we have a reasonable model, let's see how we can include predictors to the model.  One way to include predictors is to examine if the parameters of the logistic growth regression are influenced by the predictor, e.g. do the state rate parameters vary according to the predictor.   We can accomplish this by, say, having the rate for each state be a linear function of the predictor, as in rate = $\beta_0$ + $\beta_2$ predictor + $\epsilon$.  Our current model is the special case where $\beta_2$ os set to zero, thus rate = $\beta_0$ + $\epsilon$ where the intercept is the fixed effect (same value for each state) and the "$\epsilon$" is the random effect (a term associated with each state).  By expanding the model to include a predictor associated with the rate parameter we can examine if the predictor is linearly related to the rate parameter.


For illustration, I examine whether the poverty rate in each state is associated with the coefficient of the "b1" multiplier we estimated in the logistic growth model. 

```{r echo=longform}
par(pty="s")
plot(allstates$povertyperc, coef(out.logistic)[,3],xlab="State Poverty Percentage", ylab="random effect parameter b1 in logistic growth")
abline(coef(lm(coef(out.logistic)[,3] ~ allstates$povertyperc)))
par(pty="m")
```

 It looks like a slight negative association is present though the data are noisy. In principle, I could test this regression slope through a regular regression but it is better to do it in the context of the nonlinear logistic growth regression model directly. The reason is that a linear regression using the coefficients from the logistic growth as though they were an observed variable doesn't take into account the error structure of these parameters---the parameters aren't observed data but they are  estimates from another model.  By including the poverty predictor simultaneously with the logistic growth model, we can model the uncertainty much better. It would be even better to run this as a Bayesian model, which I'll add in a later version using the brm package.
 
 <!-- https://cran.r-project.org/web/packages/brms/vignettes/brms_multilevel.pdf -->
 
```{r messages=F, warning=F, results="hide", echo=longform}
out <- nlme(count ~ richfu.logistic(day.numeric, l,y+b2*povertyperc,b1,0), fixed = l+y+b1 + b2~1,
            random =  l+b1~1, data=na.omit(allstates.long.new),start=c( l=17500,y=200,b1=-.15, b2=-4.5), verbose=T,  control=nlmeControl(msMaxIter=1000,opt="nlminb",pnlsTol=.02,msVerbose=T,minScale=.02, maxIter=1000, eval.max=1000) )
```
...
qqnorm(coef(out.logistic)[,3],main="qqplot for y")
qqline(coef(out.logistic)[,3])

boxplot(coef(out.logistic)[,1], ylab="asymptote l parameter")
boxplot(coef(out.logistic)[,3], ylab="y parameter")
...
Typically I do not rush directly to models that assume a normally distributed dependent variable when I'm using counts or percentages.  There are more general models that allow different distributions than the normal distribution.  Often, for count data, the Poisson distribution is natural.   It turns out that the relatively large data set we are using begins to approach a normal distribution so we are probably ok with what I presented up until now. But still, it leaves a bad taste in my mouth knowning that I should probably be fitting these models with a  Poisson distribution.

Nonlinear mixed model regression with data that do not follow a normal distribution are extremely difficult to estimate. The gnm package in R allows such estimation, but for more serious estimation of nonlinear mixed models with a large class of distributional assumptions it is probably better to cast in a Bayesian estimation framework.  Still, here I show how would could fit such models in the frequentist setting with the gnm R package with a Poisson distribution. I'll use counts and the same form of the logistic growth model I used to illustrate the nlmer function (i.e., the form that is included in the SSlogis function). I'll show the summary of the coefficients, their tests, some residual plots, and plots showing New York and Michigan data.
...
# not clear why day.numeric as a linear predictor is needed for convergence
# need to check
out.gnm <- gnm(count ~  day.numeric + Logistic(day.numeric), family = poisson, eliminate=state.abb,
                     data = allstates.long.new, iterMax=2000)
```
...
plot(out.gnm, ask=F, which=1:2)

predmatnonlin <- data.frame(day.numeric=allstates.long.new$day.numeric,state.abb=allstates.long.new$state.abb, count=allstates.long.new$count)
predmatnonlin$prediction <- predict(out.gnm, newdata=data.frame(day.numeric=predmatnonlin$day.numeric, state.abb=predmatnonlin$state.abb),
                                     type="response")
predmatnonlin.logistic <- predmatnonlin
nonlinreg.logistic <- ggplot(predmatnonlin, aes(day.numeric,prediction,group=state.abb, color=state.abb )) +  geom_line()   
#nonlinreg.logistic
...
nydf <- predmatnonlin[predmatnonlin$state.abb=="New York",]
nydf.logistic <- nydf
ggplot(nydf, aes(day.numeric,prediction,group=state.abb, color=state.abb )) + geom_line() + geom_point(aes(x=day.numeric, y=count), col="blue") + ggtitle("New York: Actual counts in blue; logistic growth in red; Poisson distribution")

midf <- predmatnonlin[predmatnonlin$state.abb=="Michigan",]
midf.logistic <- midf
ggplot(midf, aes(day.numeric,prediction,group=state.abb, color=state.abb )) + geom_line() + geom_point(aes(x=day.numeric, y=count), col="blue") + ggtitle("Michigan: Actual counts in blue; logistic growth in red; Poisson distribution")

state.mean <- c(unlist(by(allstates.long.new$count, allstates.long.new$state.abb, mean,na.rm=T)))
state.var <- c(unlist(by(allstates.long.new$count, allstates.long.new$state.abb, var,na.rm=T)))

plot(state.mean[-c(49:50)], state.var[-c(49:50)])
abline(0,1)
```
...
# check
nycount.ts <- ts(nydf$count, start=1, end=max(nydf$day.numeric)+1, frequency = 1)
autoplot(nycount.ts) 

plot(forecast(nycount.ts), xlab="Day", ylab="count")
...
```{r message=F, warning=F, echo=longform, cache=cache, eval=F}
...
```{r gaussproc, warning=F, message=F, echo=longform}
