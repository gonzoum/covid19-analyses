...
```{asis echo=longform}
I could incorporate it by including a weight parameter (weights=varIdent(form= ~1|state.abb)), which allows each state to have its own residual variance, or change the dependent variable from raw counts to, say, per capita counts.  I'll leave that as an exercise to see whether it changes the results. 
```
...
Pending:  examine the nonlinear mixed model with a Poisson distribution
