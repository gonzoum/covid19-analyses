...
model <- trainr(Y = Y,
                X = X,
                learningrate = .5, momentum=.5, batchsize=100,
                hidden_dim = 8, use_bias=T,
                numepochs = 500, network_type="rnn")
...
plot(data$days[-c(1:3, length(data$days))], time.series.day.counts[-c(1:3)] , col = 'red', type = 'p', main = "RNN for New York State Daily Confirmed Cases\n actual counts (red & solid blue), training (green);  predicted (solid blue)", ylab = "Daily Counts", xlab="Day", xlim=c(0,length(data$days)), ylim=c(0,max(diff(data$counts))), cex.main=.7)
