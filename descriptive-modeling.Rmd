...
In the chapter [Descriptive Plots](#descplots) I showed that in the log scale the curves look  linear by state for the count/population variable.
...
Day is entered as a linear time variable (1, 2, 3, etc). Our previous plots suggest that on the log scale we are fine with linear time as a predictor but this could be checked to see if any curvature is statistically significant (e.g., include additional quadratic and cubic terms).
...
Now that we understand the slopes and intercepts, let's make some predictions.  I'll go 5 days out. 
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
  ...
  annotate(geom="text",x=max.day.numeric-2,y=-5, label="Days with observations", color="red") +
  annotate(geom="text",x=max.day.numeric+2,y=-5, label="Predictions", color="red") 
```

Taking the same data I  take the exponential of those predictions to see the percentages with their predicted curves.  The Y axis is on the percentage scale (count/population * 100).
...
coord_cartesian(clip = 'off') +  scale_x_continuous(limits=c(0,max.day.numeric+ days.ahead+3)) +
    theme(legend.position="none", plot.margin = margin(0.1, 1, 0.1, 0.1, "cm")) + geom_vline(xintercept=max.day.numeric+.5,color="red") +
  annotate(geom="text",x=max.day.numeric-2,y=2.5, label="Days with observations", color="red") +
  annotate(geom="text",x=max.day.numeric+2,y=2.5, label="Predictions", color="red") 
```
...
plot(0:max.day.numeric, richfu(0:max.day.numeric, 0, 1.23 , .000555 , 1)*19500000/100,type="l",xlab="days",ylab="counts",main="Hypothetical plot of counts following an exponential growth (tied to population of New York)")
```


Now let's estimate an exponential growth model using a nonlinear mixed model regression. This mimics the LMER model we ran earlier in these notes but rather than transforming with a log to convert the data into straight lines we model the curvature direction with an exponential function.
