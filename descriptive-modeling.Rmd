...
out.perc <- nlme(percap10000 ~ SSlogis(day.numeric, Asym, xmid,scal), fixed =  Asym + xmid + scal  ~1,
          random = Asym + xmid + scal~ 1, data=na.omit(allstates.long.new),start=c(Asym=starts[1], xmid=starts[2], scal=starts[3]), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.01,msVerbose=T,minScale=.01, maxIter=400, eval.max=400))

# out.perc <- nlme(percap10000 ~ SSlogis(day.numeric, Asym, xmid,scal), fixed =  Asym + xmid + scal  ~1,
#            random = Asym + xmid + scal~ 1, data=na.omit(allstates.long.new),start=c(Asym=64, xmid=60, scal=15), verbose=T, control=nlmeControl(msMaxIter=500,opt="nlminb",pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=400, eval.max=400))
```
...
out.logistic.nlmer <- nlmer(percap10000 ~ SSlogis(day.numeric, asym, xmid,scal) ~ asym + xmid | state.abb, data=na.omit(allstates.long.new), start=c(asym=55,xmid=54,scal=12),    control=nlmerControl(tolPwrss=.01))
...
out <- nlme(percap10000 ~ SSlogis(day.numeric, Asym, xmid, scal+b2*povertyperc), fixed = Asym + xmid + scal+b2~1,
            random =  Asym  + xmid + scal ~1, data=na.omit(allstates.long.new),
            start=c( Asym=start.vals[1],xmid=start.vals[2],scal=start.vals[3], b2=.01), verbose=T,  
            control=nlmeControl(msMaxIter=1000,opt="nlminb",tolerance=.01, pnlsTol=.001,msVerbose=T,minScale=.001, maxIter=1000, eval.max=1000) )
```
