...
As we learned last semester, polynomials can do a good job at capturing functions but they are not always easy to interpret. The third order polynomial got the pattern relatively closely. The quadratic though while it was close predicts that the counts in the early days will drop (the red curves is descreasing from day 1 to day 4) so that's not good because the data points don't see to be showing that pattern.  The nonlinear model (nls) estimated the paramters quite closely (the true multiplier was .15 and that is what we estimated; the standard eror of the residuals was .25 and it was estimated as .29). The other highlight is that the linear model on the log data (black curve) was almost indistinghisable from the true exponential pattern as estimated by the fits of the green line.  The estimates from the linear model (lm) on the log transformed data are also quite close to the true value. The intercept in the log linear model should correspond to the multiplier in the true model, which is 1, and the exponential of the intercept is .98 (exp(-.0183)). The value of the multiplier in the log linear model (.1512) is close to the true multiplier in the model I created .15.
...
You can think of this model as being similar to a balance in a savings account that builds with compound interest. At each time step, a percentage is added to the current principlem thus increasing the principle. At the subsequent time step, the principle is now higher so the interest that day will also be higher.  In the case of the covid-19 counts, each day the total grows by some percentage, like the principle in the savings account.
...
Now let's estimate an exponential growth model using a nonlinear mixed model regression. This mimics the LMER model we ran earlier in these notes but rather than transforming with a log to convert the data into straight lines we model the curvature direction with an exponential function. To simplify things I constrain the liftoff parameter to 0 and the multiplier b1 to 1.  To estimate all four parameters simultaneously we need to move to a different R package that uses a stochastic EM algorithm to estiamte this nonlinear mixed model because nlme can't handle very complex models like this one.
...
The logistic growth model starts off being very close to an exponential but eventuallly tapers off. Many biological systems behave like a logistic growth function.
...
#commented out due to varcov issues; need to check
